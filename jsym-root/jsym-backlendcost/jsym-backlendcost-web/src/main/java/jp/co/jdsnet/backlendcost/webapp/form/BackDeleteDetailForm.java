...
public class BackDeleteDetailForm implements FormInterface<BackDeleteDetailDTO>, Serializable {
  ...
  public BackDeleteDetailDTO toDTO() {
    return BackDeleteDetailDTO.builder().no(this.no).skocod(this.skocod).hjihnb(this.hjihnb)
        ...
        .chzdelkjndte(this.chzdelkjndte).fmg1(this.fmg1).fmg2(this.fmg2)
  ...
  public static BackDeleteDetailForm toForm(BackDeleteDetailDTO dto) {
    return BackDeleteDetailForm.builder().no(dto.getNo()).skocod(dto.getSkocod())
        ...
        .jucmdsseq(dto.getJucmdsseq()).jucmeiseq(dto.getJucmeiseq()).seq(dto.getSeq())
        .trncod(dto.getTrncod()).chzjkk(dto.getChzjkk()).thcbng(dto.getThcbng())
        .sjcod(dto.getSjcod()).ykncod(dto.getYkncod()).mscod(dto.getMscod())
        .tankinhjiflg(dto.getTankinhjiflg()).daikaiskbcod(dto.getDaikaiskbcod())
        .kigbng(dto.getKigbng()).trjchzsur(dto.getTrjchzsur()).tankinstekbn(dto.getTankinstekbn())
        .skrtan(dto.getSkrtan()).rmcod(dto.getRmcod()).eigcod(dto.getEigcod())
        .tercod(dto.getTercod()).skosteariflg(dto.getSkosteariflg()).chzrelno(dto.getChzrelno())
        .cpufulid(dto.getCpufulid()).wsseq7kt(dto.getWsseq7kt())
