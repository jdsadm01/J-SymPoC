...
//		));
	...
	private void editDetail(AllStockForm fmForm) {
		...
		fmForm.getDetailList().forEach(t ->{
			addln(createStringSet(new String[] {
					toText(t.getHjinm2()),
					toText(t.getSyacod()),
					toText(t.getCchhinkbn()),
					toDigit(t.getZaisur()),
					toDigit(t.getSyaknosur()),
					toDigit(t.getZaikhokhasur()),
					toDigit(t.getChzkhasur()),
					toDigit(t.getSinjuckhasur()),
					toDigit(t.getSinsyasur()),
					toDigit(t.getSekzansur()),
					toDigit(t.getKdazansur()),
					toDigit(t.getYkesyakhasur()),
					toDigit(t.getTodjucsur()),
					toDigit(t.getTodsyasur()),
					toDigit(t.getTomjucsur()),
					toDigit(t.getTomsyasur())
			}));
		});
