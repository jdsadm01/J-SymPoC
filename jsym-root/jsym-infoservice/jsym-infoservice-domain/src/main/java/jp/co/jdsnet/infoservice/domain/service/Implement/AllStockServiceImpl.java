...
public class AllStockServiceImpl implements AllStockService {
  ...
  public AllStockDTO init(String daikaiskbcod, String usrbun) {
    ...
    return AllStockDTO.builder()
        .daikaiskbcodList(lst.stream().map(t -> t.getDaikaiskbcod()).distinct().sorted()
            .collect(toList()))
        .isInServiceTime(isOnline).isFavoriteAuthority(isFavorite).build();
  }

  public AllStockDTO search(AllStockDTO dto) throws Exception {
    // サービス時間チェック
    boolean isOnline = false;
    if (CheckSharedService.SERVICETIME_ONLINE.equals(checkSharedService.checkServiceTime(
    ...
    try {
      ...
      String checkcod = "JDS".equals(dto.getUserInfo().getDaikaiskbcod()) ? dto.getKaiskbcod()
          : dto.getUserInfo().getKaiskbcod();
      kaishaEntity = kaishaMapper.selectByPrimaryKey(KaishaEntity.builder().kaiskbcod(checkcod).build());
      // checkSharedService.checkKyoten(kaishaEntity.getDaikaiskbcod(),
      // dto.getUserMap().get("usrbun"));
    } catch (Exception e) {
    ...
    int urisurrui = eigyoshobetsuHinbanUriageMapper
        .getUrisurruiSummary(kaishaEntity.getDaikaiskbcod(), hinbanEntity.getKigbng());
    ...
    // 発注情報取得
    List<Map<String, Integer>> hatchYoteList =
        hatchuMapper.selectHatchuyoteiList(kaishaEntity.getDaikaiskbcod(), hinbanEntity.getKigbng(),
            dataGetSharedService.getKaiQueryList(kaishaEntity.getDaikaiskbcod(),
                kaishaEntity.getKaiskbcod(), "JTBTRN12V"));

    // 出荷制限取得

    // 在庫情報取得
    List<EigyoshoSokoGroupVO> eigSkoList = dataGetSharedService.getEigSkoGrpList(
        kaishaEntity.getDaikaiskbcod(), kaishaEntity.getKaiskbcod(), "VHJ002", "1", "1");

    List<AllStockDetailDTO> detail = eigSkoList.stream().map(t -> {
      AZaikoEntity aZaikoEntity = aZaikoMapper.selectAZaikoSkoSum(hinbanEntity.getDaikaiskbcod(),
      ...
      return AllStockDetailDTO.builder().hjinm2(t.getHjinm2()).syacod(zaikojokenEntity.getSyacod())
          .cchhinkbn(zaikojokenEntity.getCchhinkbn()).zaisur(aZaikoEntity.getZaisur())
          .syaknosur(syaknosuu).zaikhokhasur(aZaikoEntity.getZaikhokhasur())
          .chzkhasur(aZaikoEntity.getChzkhasur()).sinjuckhasur(aZaikoEntity.getSinjuckhasur())
          .sinsyasur(aZaikoEntity.getSinsyasur()).sekzansur(aZaikoEntity.getSekzansur())
          .kdazansur(aZaikoEntity.getKdazansur()).ykesyakhasur(aZaikoEntity.getYkesyakhasur())
          .todjucsur(aZaikoEntity.getTodjucsur()).todsyasur(aZaikoEntity.getTodsyasur())
          .tomjucsur(aZaikoEntity.getTomjucsur()).tomsyasur(aZaikoEntity.getTomsyasur()).build();
    }).collect(toList());
